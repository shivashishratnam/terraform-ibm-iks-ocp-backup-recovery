...
# ############################################################################
# Deploy Data Source Connector and register cluster with IBM Backup & Recovery
# ############################################################################

module "backup_recovery" {
  source  = "terraform-ibm-modules/iks-ocp-backup-recovery/ibm"
  version = "X.Y.Z" # Replace with a release version to lock into a specific release

  # --- Cluster ---
  cluster_id                = "xxxxxxxxx" # replace with cluster ID
  cluster_resource_group_id = "xxxxxxxxx" # replace with resource group ID
  kube_type                 = "openshift" # or "kubernetes"
  connection_env_type       = "kRoksVpc"  # kRoksVpc | kIksVpc | kRoksClassic | kIksClassic

  # --- IBM Cloud credentials ---
  ibmcloud_api_key = var.ibmcloud_api_key # pragma: allowlist secret

  # --- Backup & Recovery Service instance ---
  # Option A: reuse an existing BRS instance
  existing_brs_instance_crn = "crn:v1:bluemix:public:backup-recovery:us-south:a/xxxx:xxxx::"
  # Option B: create a new BRS instance (set region + brs_instance_name instead)
  # region           = "us-south"
  # brs_instance_name = "my-brs-instance"

  # --- Connection ---
  brs_connection_name = "my-cluster-connection"
  brs_endpoint_type   = "private"
...
| <a name="input_binaries_install_path"></a> [binaries\_install\_path](#input\_binaries\_install\_path) | Filesystem path where runtime binaries (kubectl, jq, ibmcloud, backup-recovery plugin) are installed when `install_required_binaries` is true. Defaults to /tmp. Override for non-POSIX or restricted runtime environments. | `string` | `"/tmp"` | no |
...
| <a name="input_dsc_registry"></a> [dsc\_registry](#input\_dsc\_registry) | Registry for the Data Source Connector image. Use 'icr.io' for production and 'stg.icr.io' for UAT/staging. | `string` | `"icr.io"` | no |
...
| <a name="input_dsc_stabilization_wait"></a> [dsc\_stabilization\_wait](#input\_dsc\_stabilization\_wait) | How long to wait after the Data Source Connector pod reports ready before attempting source registration. The DSC needs time to establish its internal state before BRS can reach it. Expressed as a Terraform duration string (e.g., '5m', '3m30s'). | `string` | `"5m"` | no |
...
| <a name="input_source_deregistration_wait"></a> [source\_deregistration\_wait](#input\_source\_deregistration\_wait) | How long to wait during destroy after the Helm release is removed before deregistering the BRS source. BRS source deregistration is asynchronous; without this wait the connection deletion fails with 'source still in use'. Expressed as a Terraform duration string (e.g., '5m', '3m'). | `string` | `"5m"` | no |
| <a name="input_source_discovery_wait"></a> [source\_discovery\_wait](#input\_source\_discovery\_wait) | How long to wait after source registration before reading protection sources. BRS performs asynchronous cluster discovery after registration; this sleep ensures the source tree is populated. Expressed as a Terraform duration string (e.g., '5m', '3m'). | `string` | `"5m"` | no |
...
### Kubernetes RBAC: `cluster-admin` grant

This module creates a `ClusterRoleBinding` that grants the `brsagent` service account the built-in `cluster-admin` role. This is the broadest permission level available in Kubernetes and is required by the IBM Backup & Recovery agent to:

- Read and snapshot resources across all namespaces during backup
- Create and manage Velero/OADP resources for backup and restore operations
- Access persistent volume metadata cluster-wide

A least-privilege custom `ClusterRole` is not currently supported by the IBM BRS agent. Accept this as a known security trade-off and ensure the `ibm-brs-data-source-connector` namespace is treated as a privileged namespace in your cluster's admission policies.

### Security group egress rules

When `add_dsc_rules_to_cluster_sg = true`, this module adds outbound rules to the cluster's VPC security group allowing egress to `0.0.0.0/0` on ports 443, 29991, and the cluster API port. These rules are required for the Data Source Connector to reach the IBM BRS dataplane. IBM does not publish fixed CIDR ranges for the BRS dataplane, so broad egress is the supported configuration. Restrict these rules further only if your network topology allows you to determine the specific BRS dataplane IPs for your region.
...
See [CONTRIBUTING.md](./CONTRIBUTING.md) for contribution guidelines and IBM CLA requirements.
